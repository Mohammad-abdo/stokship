...
import { Search, Eye, ShoppingCart, CheckCircle, XCircle, Package } from 'lucide-react';
...
const VendorOrders = () => {
  ...
  const handleRejectOrder = async (id) => {
    ...
    if (!reason) return;
    try {
      await vendorApi.rejectOrder(id, reason);
      fetchOrders();
    } catch (error) {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Orders List ({pagination.total || filteredOrders.length})</CardTitle>
        </CardHeader>
        <CardContent>
