...
const EditProduct = () => {
  ...
  const [productImages, setProductImages] = useState([]); // Array of { id?, file, imageUrl, imageOrder, isPrimary, altText, preview }
  ...
  const fetchProduct = async () => {
    try {
      ...
      setFormData({
        ...
        metaTitleKey: product.metaTitleKey || '',
        metaTitleAr: product.metaTitleAr || '',
        metaTitleEn: product.metaTitleEn || '',
        metaDescriptionKey: product.metaDescriptionKey || '',
        metaDescriptionAr: product.metaDescriptionAr || '',
        metaDescriptionEn: product.metaDescriptionEn || '',
        metaKeywords: product.metaKeywords || '',
        slug: product.slug || '',
        canonicalUrl: product.canonicalUrl || '',
        ogTitleKey: product.ogTitleKey || '',
        ogTitleAr: product.ogTitleAr || '',
        ogTitleEn: product.ogTitleEn || '',
        ogDescriptionKey: product.ogDescriptionKey || '',
        ogDescriptionAr: product.ogDescriptionAr || '',
        ogDescriptionEn: product.ogDescriptionEn || '',
        ogImage: product.ogImage || '',
        twitterCardTitleKey: product.twitterCardTitleKey || '',
        twitterCardTitleAr: product.twitterCardTitleAr || '',
        twitterCardTitleEn: product.twitterCardTitleEn || '',
        twitterCardDescriptionKey: product.twitterCardDescriptionKey || '',
        twitterCardDescriptionAr: product.twitterCardDescriptionAr || '',
        twitterCardDescriptionEn: product.twitterCardDescriptionEn || '',
        twitterCardImage: product.twitterCardImage || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const existingImages = productImages.filter(img => img.id);
      const newImages = productImages.filter(img => !img.id);
      
      // Get current product to compare images
      const currentProduct = await adminApi.getProduct(id);
      const currentImages = currentProduct.data.data?.images || currentProduct.data.images || [];
      
      // Delete removed images
      const currentImageIds = currentImages.map(img => img.id);
      const existingImageIds = existingImages.map(img => img.id);
      const imagesToDelete = currentImageIds.filter(imgId => !existingImageIds.includes(imgId));
      ...
      // Upload new images (files or URLs)
      if (newImages.length > 0) {
        try {
          setUploadingImages(true);
          const files = newImages.filter(img => img.file).map(img => img.file);
          const imageData = newImages.map((img, index) => ({
            imageOrder: img.imageOrder || existingImages.length + index + 1,
            isPrimary: img.isPrimary || false,
            altText: img.altText || ''
          }));
          ...
          if (files.length > 0) {
            // Upload files
            await adminApi.uploadProductImages(id, {
              files,
              imageData
            });
          } else {
          ...
          showToast.warning('Product updated but images may not have been updated', 'You can update images later');
        } finally {
      ...
      for (const img of existingImages) {
        try {
          // Note: You may need to add an update endpoint for images
      ...
      // Refresh product data to show updated translations
      await fetchProduct();
      
      showToast.success('Product updated successfully', 'The product has been updated');
      // Optionally navigate back - commented out to let user see updated data
      // navigate('/stockship/admin/products');
    } catch (error) {
      console.error('Error updating product:', error);
      showToast.error('Failed to update product', error.response?.data?.message || 'Please try again');
    } finally {
      setSaving(false);
    }
  };
                          ...
                          files.forEach((file) => {
                            ...
                            reader.onload = (e) => {
                              const newImage = {
                                ...
                                isPrimary: productImages.length === 0 && !productImages.some(img => img.isPrimary)
                              };
                        ...
                        >
                          ...
                          <div className="aspect-square relative bg-gray-100">
                            {(!img.preview && !img.imageUrl) || 
                             (img.preview && img.preview.includes('via.placeholder.com')) ||
                             (img.imageUrl && img.imageUrl.includes('via.placeholder.com')) ? (
                              <div className="w-full h-full flex items-center justify-center text-gray-400 text-xs">
                                No Image
                              </div>
                            ) : (
                              <img
                                src={getImageUrl(img.preview || img.imageUrl)}
