...
const EditCoupon = () => {
  ...
  const fetchCoupon = async () => {
    try {
      ...
      setFormData({
        ...
        isVisible: coupon.isVisible !== undefined ? coupon.isVisible : true
      });
    } catch (error) {
      console.error('Error fetching coupon:', error);
      showToast.error('Failed to load coupon', error.response?.data?.message || 'Coupon not found');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showToast.success('Coupon updated successfully', 'The coupon has been updated');
      navigate('/stockship/admin/coupons');
    } catch (error) {
    ...
    <div className="space-y-6 p-6">
      ...
      >
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-2">Update coupon information</p>
        </div>
      </motion.div>
