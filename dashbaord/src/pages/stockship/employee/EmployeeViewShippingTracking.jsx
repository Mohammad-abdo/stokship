...
import { employeeApi, dealApi } from '@/lib/mediationApi';
...
const EmployeeViewShippingTracking = () => {
  ...
  const fetchTracking = async () => {
    try {
      setLoading(true);
      const response = await employeeApi.getShippingTracking(dealId);
      ...
      if (data?.deal) {
        try {
          const dealResponse = await dealApi.getDealById(dealId);
      ...
      navigate('/stockship/employee/shipping-tracking');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      ...
      >
        <div className="flex items-center gap-4">
          <button
            onClick={() => navigate('/stockship/employee/shipping-tracking')}
            className="p-2 hover:bg-gray-100 rounded-lg"
          >
            <ArrowLeft className="w-5 h-5" />
          </button>
          <h1 className="text-3xl font-bold">{t('mediation.employee.shippingTracking.title') || 'Shipping Tracking'}</h1>
        </div>
        <Card>
          <CardContent className="pt-6 text-center py-12">
            <AlertCircle className="w-16 h-16 text-gray-400 mx-auto mb-4" />
            <p className="text-gray-600 text-lg">{t('mediation.employee.shippingTracking.view.noTrackingFound') || 'No shipping tracking found for this deal'}</p>
            <button
              onClick={() => navigate('/stockship/employee/shipping-tracking')}
              className="mt-4 px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary/90"
            >
              {t('mediation.employee.shippingTracking.view.backToList') || 'Back to Tracking List'}
            </button>
          </CardContent>
        </Card>
      </motion.div>
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold">{t('mediation.employee.shippingTracking.view.title') || 'Shipping Tracking Details'}</h1>
            <p className="text-gray-600 mt-1">{t('mediation.employee.shippingTracking.view.deal') || 'Deal'}: {tracking.deal?.dealNumber || 'N/A'}</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => navigate(`/stockship/employee/deals/${dealId}`)}
            className="flex items-center gap-2 px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors"
          >
            <Eye className="w-4 h-4" />
            {t('mediation.employee.shippingTracking.view.viewDeal') || 'View Deal'}
          </button>
        </div>
      </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                {t('mediation.employee.shippingTracking.view.trackingInformation') || 'Tracking Information'}
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-medium text-gray-500">{t('mediation.employee.shippingTracking.trackingNumber') || 'Tracking Number'}</label>
                  <p className="mt-1 font-mono text-lg font-semibold">
                    {tracking.trackingNumber || t('mediation.employee.shippingTracking.view.notAssigned') || 'Not assigned'}
                  </p>
                </div>
                <div>
                ...
                {tracking.currentLocation && (
                  <div className="col-span-2">
                    <label className="text-sm font-medium text-gray-500">{t('mediation.employee.shippingTracking.currentLocation') || 'Current Location'}</label>
                ...
                {tracking.estimatedDelivery && (
                  <div>
                    <label className="text-sm font-medium text-gray-500">{t('mediation.employee.shippingTracking.view.estimatedDelivery') || 'Estimated Delivery'}</label>
                    <div className="flex items-center gap-2 mt-1">
                ...
                {tracking.actualDelivery && (
                  <div>
                    <label className="text-sm font-medium text-gray-500">{t('mediation.employee.shippingTracking.view.actualDelivery') || 'Actual Delivery'}</label>
                    <div className="flex items-center gap-2 mt-1">
              ...
              {tracking.notes && (
                <div className="pt-4 border-t border-gray-200">
                  <label className="text-sm font-medium text-gray-500">{t('mediation.employee.shippingTracking.view.notes') || 'Notes'}</label>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  {t('mediation.employee.shippingTracking.view.shippingCompany') || 'Shipping Company'}
                </CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  {t('mediation.employee.shippingTracking.view.dealInformation') || 'Deal Information'}
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-medium text-gray-500">{t('mediation.employee.shippingTracking.view.dealNumber') || 'Deal Number'}</label>
                  ...
                  {deal.negotiatedAmount && (
                    <div>
                      <label className="text-sm font-medium text-gray-500">{t('mediation.employee.shippingTracking.view.negotiatedAmount') || 'Amount'}</label>
                  ...
                  {deal.totalCartons && (
                    <div>
                      <label className="text-sm font-medium text-gray-500">{t('mediation.employee.shippingTracking.view.totalCartons') || 'Total Cartons'}</label>
                      <p className="mt-1 font-semibold">{deal.totalCartons}</p>
                    </div>
                  )}
                  {deal.totalCBM && (
                    <div>
                      <label className="text-sm font-medium text-gray-500">{t('mediation.employee.shippingTracking.view.totalCBM') || 'Total CBM'}</label>
                ...
                {deal.trader && (
                  <div className="pt-4 border-t border-gray-200">
                    <label className="text-sm font-medium text-gray-500">{t('mediation.employee.shippingTracking.view.trader') || 'Trader'}</label>
                    <div className="flex items-center gap-2 mt-1">
                ...
                {deal.client && (
                  <div className="pt-2">
                    <label className="text-sm font-medium text-gray-500">{t('mediation.employee.shippingTracking.view.client') || 'Client'}</label>
                    <div className="flex items-center gap-2 mt-1">
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  {t('mediation.employee.shippingTracking.view.statusHistory') || 'Status History'}
                </CardTitle>
              </CardHeader>
              <CardContent>
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              ...
              >
                <Eye className="w-4 h-4" />
                {t('mediation.employee.shippingTracking.view.viewDeal') || 'View Deal Details'}
              </button>
              <button
                onClick={() => navigate('/stockship/employee/shipping-tracking')}
                className="w-full flex items-center gap-2 px-4 py-2 text-left hover:bg-gray-50 rounded-lg transition-colors"
...
export default EmployeeViewShippingTracking;
