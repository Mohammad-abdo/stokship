...
export default function PublicViewOffer() {
  const { id } = useParams();
  const navigate = useNavigate();
  const { t, language, isRTL } = useLanguage();
  const { isLoggedIn, isClient } = useMultiAuth(); // Auth check
  ...
  useEffect(() => {
    // If already logged in as client, redirect to dashboard view
    if (isClient()) {
      navigate(`/stockship/client/offers/${id}`, { replace: true });
    }
    fetchOffer();
  ...
  const handleCreateDeal = async () => {
    // 1. Check if logged in as Client
    if (!isClient()) {
      // Redirect to login with return path to the AUTHENTICATED view
      navigate('/multi-login', { state: { returnUrl: `/stockship/client/offers/${id}` } });
      return;
    }

    // If already logged in (should be redirected by useEffect, but just in case)
    navigate(`/stockship/client/offers/${id}`);
  };
